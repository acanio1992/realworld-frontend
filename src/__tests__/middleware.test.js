...
describe('promiseMiddleware', () => {
  ...
  it('sets action.error=true, dispatches ASYNC_END then action on rejection (skipTracking not set)', async () => {
    const error = { response: { body: { errors: { email: ['invalid'] } } } };
    const action = { type: 'MY_ACTION', payload: Promise.reject(error) };
    promiseMiddleware(store)(next)(action);
    await new Promise(r => setTimeout(r, 0));
    // ASYNC_END must fire with the resolved body payload
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: ASYNC_END, promise: error.response.body }),
    );
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'MY_ACTION', error: true }),
    );
  });

  it('logs RESULT with the resolved value on success', async () => {
    const consoleSpy = jest.spyOn(console, 'log').mockImplementation(() => {});
    const resolved = { data: 42 };
    const action = { type: 'MY_ACTION', payload: Promise.resolve(resolved) };
    promiseMiddleware(store)(next)(action);
    await new Promise(r => setTimeout(r, 0));
    expect(consoleSpy).toHaveBeenCalledWith('RESULT', resolved);
    consoleSpy.mockRestore();
  });

  it('logs ERROR with the rejected error on failure', async () => {
    const consoleSpy = jest.spyOn(console, 'log').mockImplementation(() => {});
    const error = { response: { body: { message: 'oops' } } };
    const action = { type: 'MY_ACTION', payload: Promise.reject(error) };
    promiseMiddleware(store)(next)(action);
    await new Promise(r => setTimeout(r, 0));
